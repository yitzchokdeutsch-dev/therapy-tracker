...
import { PAGE_ACCESS, ROLE_LABELS, ROLE_COLORS } from "@/lib/permissions";
...
export default function Sidebar({ userEmail, userRole, open, onClose }: SidebarProps) {
  ...
  const visibleNav = nav.filter((item) => {
    const allowed = PAGE_ACCESS[item.href];
      ...
      >
        ...
        <div className="px-3 py-3 border-t border-surface-200 space-y-1">
          ...
          <div className="px-3 py-1.5 flex items-center justify-between">
            <div className="text-xs text-ink-400 truncate mr-2">{userEmail}</div>
            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex-shrink-0 ${ROLE_COLORS[userRole]}`}>
