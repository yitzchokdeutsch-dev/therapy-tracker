...
import { useClients, useBalances, usePaymentsForMonth } from "@/hooks";
import { useRecordPayment, useDeletePayment } from "@/hooks/mutations";
...
export default function PaymentsPage() {
  ...
  const [form, setForm] = useState({
    client_id: "", amount: "", method: "cash", reference: "", notes: "",
    payment_date: todayStr(),
  });

  const { data: clients = [], isLoading: loadingClients } = useClients();
  const { data: balances = {} } = useBalances();
  const { data: payments = [], isLoading: loadingPayments } = usePaymentsForMonth(filterMonth);

  const recordMutation = useRecordPayment(filterMonth);
  const deleteMutation = useDeletePayment(filterMonth);
  ...
  const savePayment = async () => {
    ...
    await recordMutation.mutateAsync({
      ...
      method: form.method as "cash" | "check" | "card" | "insurance" | "other",
  ...
  const filteredPayments = payments.filter((p) => {
    ...
    return cl && `${cl.first_name} ${cl.last_name}`.toLowerCase().includes(searchClient.toLowerCase());
  });
        ...
        <div>
          <div className="card p-5 mb-4">
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={savePayment}
                disabled={recordMutation.isPending || !form.client_id || !form.amount || Number(form.amount) <= 0}
              ...
              <div className="space-y-2 max-h-64 overflow-auto">
                {clientsByBalance.map((c) => (
                  <div key={c.id} className="flex items-center justify-between py-1.5 border-b border-surface-200 last:border-0">
                    <button onClick={() => setForm({ ...form, client_id: c.id })}
                  ...
                  {filteredPayments.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="table-row">
                        ...
                        <td className="table-cell font-semibold text-sm">{cl ? `${cl.first_name} ${cl.last_name}` : "Unknown"}</td>
                        <td className="table-cell"><span className="badge badge-gray">{methodLabel}</span></td>
