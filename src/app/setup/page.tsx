...
interface Therapist { id: string; name: string; email: string; phone: string; color: string; active: boolean; }
...
function TherapistsTab() {
  ...
  const openNew = () => { setForm({ name: "", email: "", phone: "", color: COLORS[therapists.length % COLORS.length] }); setEditId(null); setShowForm(true); };
  const openEdit = (t: Therapist) => { setForm({ name: t.name, email: t.email || "", phone: t.phone || "", color: t.color }); setEditId(t.id); setShowForm(true); };
  ...
  const toggleActive = async (t: Therapist) => { await supabase.from("therapists").update({ active: !t.active }).eq("id", t.id); load(); };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={openNew} className="btn-primary btn-sm">+ Add Therapist</button>
      </div>
      {showForm && (
        <div className="card p-5 mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={save} disabled={saving || !form.name.trim()} className="btn-primary btn-sm">{saving ? "Saving..." : editId ? "Update" : "Add"} Therapist</button>
            <button onClick={() => setShowForm(false)} className="btn-ghost btn-sm">Cancel</button>
          </div>
        </div>
      )}
      {therapists.length === 0 ? (
        <div className="card p-12 text-center text-ink-400">No therapists yet. Add your first one above.</div>
      ) : (
        <div className="card overflow-hidden">
          <table className="w-full">
            <thead><tr className="border-b border-surface-200">
              <th className="table-header">Therapist</th><th className="table-header">Email</th><th className="table-header">Phone</th><th className="table-header">Status</th><th className="table-header text-right">Actions</th>
            </tr></thead>
            <tbody>
              {therapists.map((t) => (
                <tr key={t.id} className="table-row">
                  <td className="table-cell"><div className="flex items-center gap-3"><span className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: t.color }} /><span className="font-semibold">{t.name}</span></div></td>
                  <td className="table-cell text-ink-500">{t.email || "\u2014"}</td>
                  <td className="table-cell text-ink-500">{t.phone || "\u2014"}</td>
                  <td className="table-cell"><span className={t.active ? "badge badge-green" : "badge badge-gray"}>{t.active ? "Active" : "Inactive"}</span></td>
                  <td className="table-cell text-right">
                    <button onClick={() => openEdit(t)} className="btn-ghost btn-sm">Edit</button>
...
function ServicesTab() {
  ...
  const load = async () => { const { data } = await supabase.from("service_types").select("*").order("name"); setServices(data || []); setLoading(false); };
  useEffect(() => { load(); }, []);

  const openNew = () => { setForm({ name: "", duration: "30", rate: "" }); setEditId(null); setShowForm(true); };
  const openEdit = (s: ServiceType) => { setForm({ name: s.name, duration: String(s.duration), rate: String(s.rate) }); setEditId(s.id); setShowForm(true); };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={openNew} className="btn-primary btn-sm">+ Add Service Type</button>
      </div>
      {showForm && (
        <div className="card p-5 mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            ...
            <div><label className="label">Rate *</label><input className="input-field" type="number" step="0.01" placeholder="125.00" value={form.rate} onChange={(e) => setForm({ ...form, rate: e.target.value })} /></div>
          </div>
          <div className="flex gap-2">
            <button onClick={save} disabled={saving || !form.name.trim() || !form.rate} className="btn-primary btn-sm">{saving ? "Saving..." : editId ? "Update" : "Add"} Service</button>
            <button onClick={() => setShowForm(false)} className="btn-ghost btn-sm">Cancel</button>
          </div>
        </div>
      )}
      {services.length === 0 ? (
        <div className="card p-12 text-center text-ink-400">No service types yet. Add your first one above.</div>
      ) : (
        <div className="card overflow-hidden">
          <table className="w-full">
            <thead><tr className="border-b border-surface-200">
              <th className="table-header">Service</th><th className="table-header">Duration</th><th className="table-header">Rate</th><th className="table-header">Status</th><th className="table-header text-right">Actions</th>
            </tr></thead>
            <tbody>
              {services.map((s) => (
                <tr key={s.id} className="table-row">
                  ...
                  <td className="table-cell text-right"><button onClick={() => openEdit(s)} className="btn-ghost btn-sm">Edit</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function FeesTab() {
  ...
  const feeDesc: Record<string, string> = { late_cancel: "Charged when a client cancels after the allowed window", no_show: "Charged when a client misses their appointment without notice" };

  if (loading) return <div className="text-ink-400 py-12 text-center">Loading...</div>;

  return (
    <div className="max-w-lg">
      <p className="text-sm text-ink-500 mb-4">These fees are automatically applied when a therapist marks a session as late cancel or no-show.</p>
